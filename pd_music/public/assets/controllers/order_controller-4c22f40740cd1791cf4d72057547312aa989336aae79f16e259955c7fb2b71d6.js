...
export default class extends Controller {
    ...
    confirmOrder(event){
        ...
        if (has_second){
            console.log(document.querySelector('input[name=secondDishRadio]:checked'));
        ...
        const create_option = fetch(url, {
            ...
            body: JSON.stringify({"update_id": update_id.value, "menu_id": menu_id.value, "date": date.value, "detail": detail.value, "option": JSON.stringify(optional), "has_second": has_second, "second_menu_id": second_menu_id, "price": price}),
          }).then(response => {
            if (response.ok) {
              return response.text();
            }
    ...
    selectDate(event){
        ...
        const get_option = fetch(url+"?date="+date.value).then(response => {
            if (response.ok) {
              return response.text();
            }
          });
    ...
    editOrder(event){
        ...
        const get_option = fetch(url+"?id="+edit_id+"&date="+date.value).then(response => {
            if (response.ok) {
              return response.text();
            }
          });
