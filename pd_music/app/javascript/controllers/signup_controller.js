...
export default class extends Controller {
    ...
    onPostSuccessUser(event){
        ...
        if (!/[0-9]/.test(room.value) && room.value != ""){
            this.invalid_value(room);
            this.alert("error", "Please fill room with number only");
        }else{
            this.valid_value(room);
            valid_room = true
        }

        if (!/[0-9]/.test(student_no.value) && student_no.value != ""){
            this.invalid_value(student_no);
            this.alert("error", "Please fill student no. with number only");
        }else{
            this.valid_value(student_no);
            valid_student_no = true
        }
