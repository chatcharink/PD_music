...
export default class extends Controller {
    ...
    createSubject(event){
        ...
        if (subject.value == ""){
            ...
            subject.style.border = "1px solid #dee2e6";
            subject.style.background = "#fff";
        }
    ...
    createEvent(event){
        ...
        if (event_name.value == ""){
            ...
            event_name.style.border = "1px solid #dee2e6";
            event_name.style.background = "#fff";
        }
    ...
    searchSubject(event){
        let search = document.getElementById("txt-search-subject");
        let url = event.params["url"];
        if (event.params["clear"] == "true" || event.params["clear"]){
            search.value = "";
        }
        if (search.value != "" || event.params["clear"]){
            const search_subject = fetch(url, {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-Token": this.getCsrfToken()
                },
                body: JSON.stringify({"subject": search.value, "type": "subject"})
            }).then(response => {
                if (response.ok) {
                    return response.text();
                }
    ...
    searchEvent(event){
        let search = document.getElementById("txt-search-event");
        let url = event.params["url"];
        if (event.params["clear"] == "true" || event.params["clear"]){
            search.value = "";
        }
        if (search.value != "" || event.params["clear"]){
            const search_subject = fetch(url, {
                ...
                body: JSON.stringify({"event": search.value, "type": "event"})
            }).then(response => {
                if (response.ok) {
                    return response.text();
                }
