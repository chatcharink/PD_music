...
export default class extends Controller {
    ...
    setDeleteId(event){
        ...
        let name = event.params["meal"];

        document.getElementById("delete-order-name").innerHTML = " : "+name;
        document.getElementById("confirm-delete-order").setAttribute("data-id", id);
        document.getElementById("confirm-delete-order").setAttribute("data-name", name);
    }

    deleteOrder(){
        let id = document.getElementById("confirm-delete-order").getAttribute("data-id");
        let name = document.getElementById("confirm-delete-order").getAttribute("data-name");
        let application_path = document.getElementById("application-path");
        let url = application_path.value+"/food/order";
        ...
        delete_restaurant.then((data) => {
            try{
                ...
                if (result["status"] == "success"){
                    window.location.replace(application_path.value+"/food/order");
                }else { 
                    this.alert(result["status"], result["message"]);
                }
            } catch {
                // $("#close-confirm-delete-order").click();
                // window.location.replace(application_path.value+"/food/order")
                // this.alert("success", "Delete order meal : "+name+" successfully");
