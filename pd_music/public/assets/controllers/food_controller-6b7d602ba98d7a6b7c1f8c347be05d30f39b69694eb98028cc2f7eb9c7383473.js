...
export default class extends Controller {
    ...
    minimizeCoupon(){
        $("#div-ticket").hide("slide", { direction: "right" }, 1000);
        $("#btn-ticket-amount").css("display", "block");
    }

    expendCoupon(){
        $("#div-ticket").show("slide", { direction: "left" }, 1000);
        $("#btn-ticket-amount").css("display", "none");
    }
    ...
    createRestaurant(event){
        ...
        let update_id = document.getElementById("update-restaurant-id");
        ...
        if (!arr_valid.includes(false)){
            ...
            create_restaurant.then((data) => {
                try{
                    ...
                    if (update_id.value == "" || update_id.value == null){
                        this.alert("success", "Add restaurant : "+name.value+" successfully");
                    } else {
                        this.alert("success", "Update restaurant : "+name.value+" successfully");
                    }
    ...
    editRestaurant(event){
        ...
        let name = event.params["restaurantName"];
        let status = event.params["restaurantStatus"];
        let telephone = event.params["restaurantTelephone"];
        let line_id = event.params["restaurantLine"];
        let img = document.getElementById("img-restaurant-"+id);

        document.getElementById("avatar_profile").src = img.src;
        ...
        document.getElementById("btn-create-restaurant").value = "Update";
    }

    clearDefault(){
        let application_path = document.getElementById("application-path");
        let img = application_path.value+"/pictures/no_image.jpg"
        document.getElementById("avatar_profile").src = img;
        ...
        document.getElementById("btn-create-restaurant").value = "Create";
    }
    ...
    setDeleteId(event){
        ...
        document.getElementById("delete-restaurant-name").innerHTML = " : "+name;
        document.getElementById("confirm-delete-restaurant").setAttribute("data-id", id);
        document.getElementById("confirm-delete-restaurant").setAttribute("data-name", name);
    }

    deleteRestaurant(){
        let id = document.getElementById("confirm-delete-restaurant").getAttribute("data-id");
        let name = document.getElementById("confirm-delete-restaurant").getAttribute("data-name");
        let application_path = document.getElementById("application-path");
        let url = application_path.value+"/food/restaurants"
        const delete_restaurant = fetch(url, {
            method: 'DELETE',
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-Token": this.getCsrfToken()
        ...
        delete_restaurant.then((data) => {
            try{
                let result = JSON.parse(data);
                this.alert(result["status"], result["message"]);
            } catch {
                $("#close-confirm-delete-restaurant").click();
                $(".div-restaurant").html("");
                $(".div-restaurant").html(data);
                this.alert("success", "Delete restaurant : "+name+" successfully");
