...
export default class extends Controller {
    connect() {
        ...
        if (table != "undefined"){
            let columnNames = [{"data": "picture"}, {"data": "name"}];
            let columns = document.getElementsByClassName("thead-date-summary");
    ...
    selectTimesPerDay(){
        ...
        for(let i = 0; i < parseInt(time_order.value); i++){
            div_meal[0].appendChild(this.generateMealInput(i+1));
        }
    }
    ...
    generateMealInput(num){
        let div = '<div class="mb-3 row">'+
                ...
                '<div class="col-sm-8">'+
                '<input type="text" class="form-control" id="mealName'+num+'" value="" />'+
                '</div>'+
                '</div>'
        return div
    }
