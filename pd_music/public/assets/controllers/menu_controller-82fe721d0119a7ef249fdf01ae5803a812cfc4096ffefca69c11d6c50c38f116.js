...
export default class extends Controller {
    ...
    showMenu(){
        ...
        let permission = document.getElementById("permission-edit");
        let table = $("#table-menu-list").DataTable({
            ...
            rowCallback: function (row, data) {
                if (permission.value == "true"){
                    for (let i = 0; i < $('td', row).length-1; i++ ){
                        $('td', row)[i].setAttribute("data-bs-toggle", "modal");
                        $('td', row)[i].setAttribute("data-bs-target", "#addMenuBackdrop");
                        $('td', row)[i].setAttribute("data-action", "click->menu#editMenu");
                        let menu = data.menuName.split(" ")[0];
                        $('td', row)[i].setAttribute("data-menu-menu-param", menu);
                        $('td', row)[i].setAttribute("data-menu-restaurant-id-param", restaurant_id.value);
                    }
                }
    ...
    selectOption(event){
        ...
        try{
            ...
            if (has_div == null){
                let last_div = document.getElementById("div-set-second-dish");
    ...
    changeStatus(event){
        let id = event.params["id"];
        let url = event.params["url"];
        let menu = document.getElementById("menu-status-"+id);
        ...
        const change_status = fetch(url, {
            ...
            body: JSON.stringify({ menu_id: id, status: menu.checked })
        }).then(response => {
            if (response.ok) {
                return response.text();
            }
